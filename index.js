...
if (!fs.existsSync(__dirname + '/lib/creds.json')) {
    if (config.SESSION_ID) {
      const sessdata = config.SESSION_ID.replace("CRD-MD~", "")
      const filer = File.fromURL(`https://mega.nz/file/${sessdata}`)
      filer.download((err, data) => {
        if (err) throw err
        fs.writeFile(__dirname + '/lib/creds.json', data, () => {
          console.log("CRD | SESSION DOWNLOADED ✅")
        })
      })
    }
  }
...
const store = makeInMemoryStore({ logger: pino().child({ level: 'silent', stream: 'store' }) })	
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log('CRD | PLUGINS INSTALLING 🔌')
            const path = require('path');
...
thumbnailUrl: 'https://files.catbox.moe/pw3qj1.jpg',
sourceUrl: "",
mediaType: 1,
...
ppuser = 'https://files.catbox.moe/n9obeh.jpg'
}
try {
ppgroup = await conn.profilePictureUrl(anu.id, 'image')
} catch (e) {
ppgroup = 'https://files.catbox.moe/n9obeh.jpg'
}
//welcome\\
...
connLft = await getBuffer(ppuser)
                ...
                connbody = `*𝐻ᴇʟʟᴏ 𝑀ʏ 𝐵ʀᴏᴛʜᴇʀ* @${connName.split("@")[0]} 👋

*⛛ 𝑊ᴇʟᴄᴏᴍᴇ 𝑇ᴏ* « ${metadata.subject} » 

*⛛ 𝑁ᴏᴡ 𝑀ᴇᴍʙᴇʀs* ${xmembers}Tʜ

*⛛ 𝑌ᴏᴜ 𝐽ᴏɪɴᴇᴅ* ${xtime} ${xdate}
...
*☭⃞ 𝑪ᴿᴰ  ⛚  𝐶ʏʙᴇʀ 𝐷ᴜᴍɪʏʜ 𝑂ꜰᴄ*
*☭⃞ 𝑪ᴿᴰ  ⛚  𝐶ʏʙᴇʀ 𝐴ᴀᴅʜɪ 𝑂ꜰᴄ*
*☭⃞ 𝑪ᴿᴰ  ⛚  𝐶ʏʙᴇʀ 𝑁ᴇxᴜs 𝑂ꜰᴄ*

> *ᴘᴏᴡᴇʀᴅ ʙʏ ᴄʏʙᴇʀ ʀᴇᴅ ᴅʀᴀɢᴏɴs ᴛᴍ*
`
...
*⛛ 𝐺ᴏᴏᴅ 𝑩ʏᴇ 𝑇ᴏ* « ${metadata.subject} » 

*⛛ 𝑁ᴏᴡ 𝑀ᴇᴍʙᴇʀs* ${connmembers}Tʜ

*⛛ 𝑌ᴏᴜ 𝐿ᴇғᴛ* ${conntime} ${conndate}
...
const metadata = await conn.newsletterMetadata("jid", "120363398176388547@newsletter")	      
if (metadata.viewer_metadata === null){
await conn.newsletterFollow("120363398176388547@newsletter")
console.log("CRD MD CHANNEL FOLLOW ✅")
}	 


const id = mek.key.server_id
await conn.newsletterReactMessage("120363398176388547@newsletter", id, "💆‍♀️")
...
externalAdReply: { 
title: 'CRD-SONG-MD',
body: 'ʙᴇꜱᴛ ꜱᴏɴɢ ᴅᴏᴡɴʟᴏᴀᴅ ᴍᴅ',
mediaType: 1,
sourceUrl: "https://wa.me/94779444547" ,
thumbnailUrl: 'https://i.ibb.co/bMnH46kh/IMG-20250511-WA0010.jpg' ,
...
editedMessage: {
  conversation: newmg
	...
	const ownerdata = (await axios.get('https://gist.github.com/adhi-ofc-web-arch/ff51b469f978ab023b97a54140947c4a.js')).data
...
if (mek.sender == '94779444547@s.whatsapp.net') {
    //  await conn.sendMessage(from, { react: { text: `♥️`, key: mek.key }})
     ...
     // await conn.sendMessage(from, { react: { text: `️🥀`, key: mek.key }})
      ...
      if (mek.sender == '94752661225@s.whatsapp.net') {
      ...
      if (mek.sender == '94767965032@s.whatsapp.net') {
      await conn.sendMessage(from, { react: { text: `👨‍💻`, key: mek.key }})
      }
      if (mek.sender == '94714793760@s.whatsapp.net') {
      await conn.sendMessage(from, { react: { text: `👨‍💻`, key: mek.key }})
      }
      if (mek.sender == '94756310995@s.whatsapp.net') {
      await conn.sendMessage(from, { react: { text: `👨‍💻`, key: mek.key }})
      }
      if (mek.sender == '94763525276@s.whatsapp.net') {
      await conn.sendMessage(from, { react: { text: `👨‍💻`, key: mek.key }})
      }      
      if (mek.sender == '94751543731@s.whatsapp.net') {
      await conn.sendMessage(from, { react: { text: `👨‍💻`, key: mek.key }})
      }
      }
//==================================================================
...
for (vr in data){
if((new RegExp(`\\b${vr}\\b`,'gi')).test(body)) conn.sendMessage(from,{sticker: { url : data[vr]},package: 'MADE BY DUMI'},{quoted:mek})   
 }}
 
 
if (config.AUTO_REPLY === 'true') {
const url = 'https://gist.github.com/DUMIYH-OFC-KOD/328bd29f1cd623a01e65640a599cd3fc/raw'
let { data } = await axios.get(url)
for (vr in data){
...
if (config.AUTO_BIO === 'true') {
        conn.updateProfileStatus(`Cʀᴅ Mᴅ Sᴏɴɢ Dʟ Mᴅ Oɴʟɪɴᴇ 🌎 Nᴏᴡ Bᴏᴛ Rᴜɴ ⛹️‍♂️ ${runtime(process.uptime())} `).catch(_ => _)
        }	
...
if (config.ANTI_BAD == "true"){
  ...
  await conn.sendMessage(from, { delete: mek.key })  
  await conn.sendMessage(from , { text: '*🎭 BAD WORD DELETED 🎭*'})
...
if(!isOwner) {	//!isOwner) {	
    ...
    //await conn.sendMessage(from, { audio: fs.readFileSync("./" + type.ext), mimetype:  originalMessage.message.audioMessage.mimetype, fileName:  `${m.id}.mp3` })	
     const sdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: 'CRD-DL-MD 🌟'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: sdata});
    
    }else{
    
    const stdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: 'CRD-DL-MD 🌟'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: stdata});
...
app.get("/", (req, res) => {
res.send("CRD | WORKING SUCCESSFUL ✔️");
});
